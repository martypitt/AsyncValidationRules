package com.inversion.model.validation
{
	...
	public class ErrorTipManager
	{
		...
		// maps the popUps to the validator
		...
		/**
		 * Unregisters the validator and removes the MOVE and RESIZE listeners on the popUp.
		 ...
		 */
		public static function unregisterValidatorOnPopUp(validator:Validator, popUp:UIComponent,
														  validateExistingErrorTips:Boolean = false):void {
		...
		private static function targetMoved(event:Event):void {
			...
			if (popUps[target] != null) {
				var validator:Validator = (popUps[target] as Validator);
				target = (validator.source as DisplayObject);
			}
			if (target is UIComponent) {
				// need to wait for resize to finish
				UIComponent(target).callLater(function():void {
